...
Para testar o GCC, compilamos os programas em C para Assembly MIPS utilizando a diretriz \textit{-Oi} (onde $i$ é um número entre 0 e 3 ou a letra $s$), que é responsável por otimizar o código gerado pelo compilador. Paramos a compilação no código de gerar o Assembly pela diretriz \textit{-S} e adaptamos o código gerado para rodar em Assembly. Sendo assim, podemos comparar o quanto cada otimização pode melhorar o nosso código original:

\begin{itemize}
    \item Sem otimizações: 1840 instruções em 3822 bytes
    \item Otimização \textit{-O1}: 3359 bytes
    \item Otimização \textit{-O2}: 3183 bytes
    \item Otimização \textit{-O3}: 3169 bytes
    \item Otimização \textit{-Os}: 2993 bytes
\end{itemize}
